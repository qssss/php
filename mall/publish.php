...
if(!empty($_POST['name'])) {
    ...
    $now = $_SERVER['REQUEST_TIME'];//获取当前时间
    ...
    // 入库处理
    $sql = "insert into goods(name,price,des,content,pic,user_id,create_time,update_time,view) values('$name',$price,'$des','$content','$imgUrl',$userId,$now,$now,0)";
    echo $sql;
    $res = mysql_query($sql);
    if($res) {
        //插入成功
